...
# injecting aa-mirror-rs into all SD card images
FROM alpine AS stage-sdcards
RUN apk --no-cache add xz
WORKDIR /root
COPY --from=stage-rust /usr/src/app/target/arm-unknown-linux-gnueabihf/release/aa-mirror-rs .
COPY --from=stage-rust /usr/src/app/pi0w/arm-unknown-linux-gnueabihf/release/aa-mirror-rs ./aa-mirror-rs-0w
ADD contrib/injector.sh .
ADD contrib/S93aa-mirror-rs .
