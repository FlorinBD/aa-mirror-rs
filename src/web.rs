...
async fn download_handler(
    ...
    tokio::spawn(async move {
        ...
        let glob_patterns = vec!["/var/log/aa-mirror-*log", "/var/log/messages"];
...
pub async fn upload_cert_bundle_handler(
    ...
    let old_path = extract_to.join("aa-mirror-rs");
    ...
    let certs_dir = Path::new("/tmp/aa-mirror-rs");
    ...
    let mut entries = match fs::read_dir(&certs_dir).await {
        ...
        Err(err) => {
            return (
                StatusCode::BAD_REQUEST,
                format!(
                    "Missing expected 'aa-mirror-rs/' directory in archive: {}",
                    err
                ),
            );
        }
