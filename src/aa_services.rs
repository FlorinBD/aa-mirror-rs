...
impl ServiceManager {
    ...
    async fn handle_hu_message(&mut self, mut pkt: Packet) -> Result<()> {
        if pkt.channel == 0
        {
            ...
            {
                //VersionRequest  can't be parsed because is not a ProtoBuf structure
                let data = &pkt.payload[2..]; // start of message data, without message_id
                let major = u16::from_be_bytes([data[0], data[1]]);
                let minor = u16::from_be_bytes([data[2], data[3]]);
                info!("{:?} HU VersionRequest: Major={}, Minor={} received, sending VersionResponse back...",self.srv_type,major,minor);
    ...
    async fn start_adb_servers(&mut self) -> Result<()> {
        ...
        self.start_adb_server.send(SCRCPYParams{audio:self.sdr_audio_codec_params.clone(), video:self.sdr_video_codec_params.clone()})?;
