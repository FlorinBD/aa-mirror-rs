...
![Hardware overview](images/aa-mirror-rs.webp)
...
The latest stable SD card images are available on the [Releases page](https://github.com/manio/aa-mirror-rs/releases).
...
Using the web interface, you can configure all settings that are also available in `/etc/aa-mirror-rs/config.toml`:
...
Default directory where the keys are search for is: `/etc/aa-mirror-rs/`, and the following file set needs to be there:<br>
- hu_key.pem
- hu_cert.pem
- md_key.pem
- md_cert.pem
...
The idea of using this feature with other cars started here: https://github.com/manio/aa-mirror-rs/issues/19 in February 2025.
...
level data to `aa-mirror-rs`, and overall it makes correct estimates for my car.

`aa-mirror-rs` has an embedded REST server for obtaining battery data from any source (I am using a slightly modified version of the
...
`aa-mirror-rs` can be configured to execute a specific data collection script when Android Auto starts and needs the battery level data, and also when it stops.
...
You can test and use **aa-mirror-rs** with Google's **Desktop Head Unit (DHU)** in two ways:

#### 1. Using a physical Raspberry Pi

This method allows you to test with actual hardware and `aa-mirror-rs` running directly on the device. The flow then looks like this:

**[📱 Android Phone] ⇄ [📶 BT+WiFi] ⇄ [📟 RPi with aa-mirror-rs] ⇄ [🔌 USB] ⇄ [🖥️ PC with DHU]**
...
[Mon Aug 25 15:24:43 2025] usb 3-1: Product: aa-mirror-rs (Raspberry Pi Zero 2 W Rev 1.0)
...
If you want to run the `aa-mirror-rs` application itself for development or functional testing — without using a physical Raspberry Pi — you can build and run it **natively** on the same platform where DHU is running (Linux/macOS).
...
**[📱 Android Phone] ⇄ [🔌 USB] ⇄ [💻 PC with aa-mirror-rs] ⇄ [🖥️ DHU]**

To make this work, follow these steps:

1. In the `aa-mirror-rs` configuration:
   - Enable the `dhu` option
   - Enable the `wired` option and provide your phone's **VID:PID** (vendor ID and product ID)
2. Compile and launch `aa-mirror-rs` on your host
3. Connect your Android phone via USB  
   *(Note: you may need to unlock the screen after connecting)*
4. `aa-mirror-rs` should detect and connect to the phone, then wait for DHU to connect
...
Default startup config file is `/etc/aa-mirror-rs/config.toml`.
...
- `connect`<br>
By default without this option the aa-mirror-rs is starting but it is only visible as a bluetooth dongle, to which you have to connect manually from your phone to
...
If you set this option to specific `MAC_ADDRESS` where MAC_ADDRESS is the MAC of your phone (bluetooth), then the aa-mirror-rs will try to connect only to this specified device
...
While the aa-mirror-rs binary is typically used as part of the prebuilt system image, it can also be run manually:

```
Usage: aa-mirror-rs [OPTIONS]

Options:
  -c, --config <CONFIG>         Config file path [default: /etc/aa-mirror-rs/config.toml]
...
>
> If you plan to run **aa-mirror-rs** on your own system (outside of the provided system images), be aware that additional **custom kernel modules** are required for full functionality. These modules are not upstreamed and must be built separately.
...
Please keep this in mind when creating your own custom solution with `aa-mirror-rs`.
